...
const getLeaderboard = async (req, res) => {
  try {
    ...
    // Build leaderboard from all users so visitors can see rankings even before login.
    const leaderboardData = await User.aggregate([
      ...
      },
      {
        $addFields: {
          totalCO2: {
            $ifNull: [{ $arrayElemAt: ["$weeklyStats.totalCO2", 0] }, 0],
          },
          activityCount: {
            $ifNull: [{ $arrayElemAt: ["$weeklyStats.activityCount", 0] }, 0],
          },
        },
      },
      {
      ...
      {
        $sort: { totalCO2: 1, createdAt: 1 }, // Lower emissions = better rank
      },
    ]);
